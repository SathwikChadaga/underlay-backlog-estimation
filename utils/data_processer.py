...
class DataProcessor:
    ...
    def scale_train(self, x_train, y_train, is_x_sequenced = True):
        if(is_x_sequenced):
        ...
        # all_max = torch.max(torch.concat((self.y_max.unsqueeze(dim=-1), self.x_max[:2].unsqueeze(dim=-1)), axis=-1), axis=1).values
        # self.y_max = all_max
        # self.x_max[:2] = all_max
